/*
  ...
  A  partir da biblioteca Emon.h criado por Trystan Lea.
  ...
  e possui comentários em português do Brasil em alguns pontos,
...
//--------------------------------------------------------------------------------------
void VIP::voltage(unsigned int _inPinV, double _VCAL, double _PHASECAL)
{
  inPinV = _inPinV;
  VCAL = _VCAL;
  PHASECAL = _PHASECAL;
...
void VIP::current(unsigned int _inPinI, double _ICAL)
{
  inPinI = _inPinI;
  ICAL = _ICAL;
...
//--------------------------------------------------------------------------------------
// emon_calc procedure  //procedimento de cálculo da biblioteca
// Calculates realPower,apparentPower,powerFactor,Vrms,Irms,kWh increment
// From a sample window of the mains AC voltage and current.
// The Sample window length is defined by the number of half wavelengths or crossings we choose to measure.
//--------------------------------------------------------------------------------------
void VIP::calcVI(unsigned int crossings, unsigned int timeout)
{
#if defined emonTxV3
  ...
  while (st == false)                                //the while loop...
  {
    ...
    startI = analogRead(inPinI);
    if ((startV < (ADC_COUNTS * 0.55)) && (startV > (ADC_COUNTS * 0.45))) {
      st = true; //check its within range
  /* if (startI == startV) {                                                  //está logica não funcionou, era para determinar o sinal do fator de potência
        sfp = 1;
      }
      else {
        sfp = 2;
...
//--------------------------------------------------------------------------------------
double VIP::calcIrms(unsigned int Number_of_Samples)
  {
...
long VIP::readVcc() {
  ...
  ADMUX = _BV(REFS0) | _BV(MUX4) | _BV(MUX3) | _BV(MUX2) | _BV(MUX1);
  ADCSRB &= ~_BV(MUX5);   // Without this the function always returns -1 on the ATmega2560 http://openVIP.org/emon/node/2253#comment-11432
  ...
  result = ADCL;
  result |= ADCH << 8;
  result = READVCC_CALIBRATION_CONST / result;  //1100mV*1024 ADC steps http://openVIP.org/emon/node/1186
  return result;
